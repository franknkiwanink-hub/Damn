...
- ~~OAuth onboarding modal~~ — resolved in Step 14: no such modal exists
  in the original to port (`__openOauthSetup` is dead code there), the
  real flow auto-derives username/avatar and goes straight into the
  onboarding tour, which is now built. Nothing further needed here.
